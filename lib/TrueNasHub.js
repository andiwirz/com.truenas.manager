...
const { get, round, percent, formatUptime, shortVersion } = require('./util');
...
class TrueNasHub extends EventEmitter {
  ...
  async _pollFast() {
    ...
    const [pools, bootPool, services, apps, vms, alerts, interfaces] = await Promise.all([
  ...
  _cpuUsageFromMean(mean) {
    if (Number.isFinite(Number(mean.cpu))) return round(mean.cpu, 1);

    if (Number.isFinite(Number(mean.idle))) {
  ...
  getDiskTemperature(disk) {
    if (!disk) return null;
    const value = this.data.diskTemperatures[disk.name];
  ...
  _setUnavailable(err, i18n = null) {
    ...
    if (this.available) {
      this.available = false;
      this.emit('unavailable', this.lastErrorI18n);
    }
  }
